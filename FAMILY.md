...
The same shape as the DSP family and a different vendor. Seta's two coprocessors
are a NEC uPD96050 carrying Seta's microcode, so what this repository models is
the interface Seta wrapped it in, and the processor is
[nec-upd7725-96050-python](https://github.com/gufranco/nec-upd7725-96050-python),
consumed as a submodule.

Seta published nothing. The top rung of the authority ladder is empty and the
record says so; what is known about the handshake comes from the cartridge's own
code, read out by
[snes-driver-python](https://github.com/gufranco/snes-driver-python).

The microcode is not carried, for the same reason it is not carried next door.

It is a part rather than a clocked part, for the same reason.
